...
UI.innerHTML = `<div id="GUI" class="GUI" style="transform: translateX(0px) translateY(-32px); opacity: 0.85; font-family: sans-serif; height: 215px; width: 124px; background: rgb(10, 10, 10); position: absolute; border-radius: 5px; display: grid; place-items: center; color: white; font-size: larger; top: 104px; left: 577px; position:absolute; z-index: 99999;">
    <div class="image">
    <img src="https://res.cloudinary.com/dc5lzhoi8/image/upload/v1675299746/icon_gyzrc9.ico" class="center">
    <h1 class="title"><center>Chase 1.0</center>
    </h1>
<style>
/* latin-ext */
@font-face {
...
.center {
  ...
  margin-right: auto;
    ...
    <button onclick="fetch('https://raw.githubusercontent.com/proxyplu/Nullify/main/Data/hecks/skip.js').then(r => r.text()).then(r => eval(r))"
    class="button">Skip Lesson</button>
    
    <button onclick="fetch('https://raw.githubusercontent.com/proxyplu/Nullify/main/Data/hecks/minfarm.js').then(r => r.text()).then(r => eval(r))" 
    class="button">Min Farmer</button>
    
    <button onclick="fetch('https://raw.githubusercontent.com/proxyplu/Nullify/main/Data/hecks/timer.js').then(r => r.text()).then(r => eval(r))" 
    ...
    <br>
    <br>
    <button onclick="fetch('https://raw.githubusercontent.com/proxyplu/Nullify/main/Data/misc/misc.js').then(r => r.text()).then(r => eval(r))" 
...
const modifedScript = "https://raw.githubusercontent.com/proxyplu/Nullify/main/Data/appchunk/app-21.chunk.js";
...
if (scriptLocationOnLoad == originalScript) { // checks if the script has already been loaded, if it has page gets reloaded
    try { radiantModal("Error!", "Please run ChaseMenu before opening lessons.", goToDashboard) } // unused as of right now
    catch (error) { alert('Please run ChaseMenu before opening lessons. Reloading...'), location.reload() }
} else {
    // override the script
    fetch(modifedScript).then(r => r.text()).then(r => eval(r))
}
} else { // reloads i-ready if a lesson is detected
  alert('Please run ChaseMenu before opening lessons. Reloading...'), location.reload()
}
} else { // prevents Radiant from opening if it's already open
  console.log('ChaseMenu is already open.')
}
