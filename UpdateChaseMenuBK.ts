...
.cheat:hover {
    ...
    header.innerHTML = `ChaseMenu Blooket <span style="font-size: 0.75rem">v1.0</span>`;
    ...
    minimize.style.background = 'blue';
    ...
    body.innerHTML = (`<span id="curPageEl">${getSite(true) ? `Current gamemode: ${getSite(true)}` : 'No game detected'}</span><br><span>(Press J to hide)</span><br>`);
    ...
    footer.innerHTML = (`<span>GUI by Chase<br>Cheats by Jordan<a style="color: Black"</a></span>`);
    ...
    function toggleHidden(e) {
        e.code == 'KeyJ' && (GUI.hidden = !GUI.hidden)
    };
